...
class FromCSVEctractor(DataExtractor):
    '''
    This class is responsible for creating data frame out from collected labolatory experiment in CSV file format.
    '''
    df_info: list = {}

    def __init__(self, file_path) -> None:
        '''
        Initialising class and creating pandas dataframe with valuable data and its discriptive dictionary.
        '''
        self.file_path = file_path
    
    def extract_data(self):
        df = pd.read_csv(filepath_or_buffer=self.file_path, sep=",", header=0, usecols=["x-axis", "3", "4"])
...
class FromH5Extractor(DataExtractor):
    '''
    This class is responsible for creating data frame out from collected labolatory experiment in H5 file format.
    '''
    df_info: list = {}
